...
The production scope is Explorer second jobs at levels 30-38, beginning with the real job-instructor opener (`2300`-`2304`) and ending when `2336` completes. Optional extermination quest `2337` is outside the story completion contract; `2338` and `2342` are recovery-only. After second job, World Director offers this as an optional authored plan alongside other activities; it is not an automatic successor.

After story completion, `mushroom-kingdom-yeti-farm` may run up to ten rematches and stops early when the exact Pepe weapon category selected by the Agent's build is obtained. A ten-run miss creates a 24-hour cooldown before a new campaign. `mushroom-kingdom-pepe-scroll` performs one bounded repeat of quest `2337`, selects the exact matching scroll reward, and attempts it on that weapon. Both plans stop being eligible at level 39.
...
For the shortened observation run, use `!mushroomtest start ten-percent [seed]`. All 12 level-30 second jobs begin at the Mushroom Kingdom entrance with their family-specific entry quest active and recommendation letter `4032375` ready to submit; `2312` is then accepted normally. Each Agent must personally obtain `ceil(authored requirement / 10)` items before the harness supplies only the missing turn-in items. One-item objectives, the three colored Yetis, and the Prime Minister remain real one-kill/one-item actions. The harness deliberately removes the first Killer Mushroom Spore and Royal Seal so recovery quests `2338` and `2342` must restore them. At the top-up boundary the harness records only the EXP and mesos earned since that collection quest began, then grants nine times those gains after the ordinary quest submission succeeds; fixed quest rewards are not multiplied.
...
| Wedding assault | `2330` | Match up to three exact-progress participants, enter `KingPepeAndYetis`, receive one uniformly random `3300005`, `3300006`, or `3300007`, and re-enter through duplicate rolls until all three are credited |
