...
- Activation precomputes copied skill, binding, inventory, and slot-limit presentation inputs by profile owner/version. Cached inventory entries remain reusable `Item` snapshots; every refresh creates fresh one-shot `ModifyInventory` operations because the packet encoder clears those wrappers after encoding. Agent movement and combat caches are warmed before switching is enabled.
...
- Solo and Double activation preload each profile with the other profile's missing cross-job, non-beginner, non-GM skill IDs. Missing records are marked as session-borrowed and sent in one multi-skill `UPDATE_SKILLS` packet. Same-job missing skills are not synthesized because passive skill lookups are not routed through the active-skill policy and could otherwise change server-side stats. For the normal cross-job Partner use case, switches therefore avoid skill add/remove packets; any original skills whose level/master-level/expiration differs between the two profiles are consolidated into one multi-skill packet. Normal `Character.changeJob` advancement behavior is unchanged.
- Borrowed union skills cannot be executed by players or Agents and cannot receive SP. A stale, already-open skill window receives a normal rejection message instead of triggering packet-edit autoban/disconnect. Successful SP assignment to an original skill during either active mode immediately updates the other profile's borrowed copy and the client-visible state; assigning the first point to a new cross-job skill adds that skill to the union. The borrowed marker follows the profile binding, not the actor.
...
- Rapid trigger casts emit at most one cooldown chat message per cooldown window. Later casts remain handled and re-enable actions without flooding chat or rejection logs.
...
Each presentation category can be isolated with `PRESENT_JOB`, `PRESENT_STATS`, `PRESENT_SKILLS`, `PRESENT_KEY_BINDINGS`, `PRESENT_EQUIPMENT`, `PRESENT_INVENTORY`, `PRESENT_PETS`, `PRESENT_BUFFS`, `PRESENT_DISEASES`, `PRESENT_COOLDOWNS`, `PRESENT_PUBLIC_LOOK`, and `PRESENT_SWITCH_EFFECT`. They default to `true`. These are diagnostic switches for profiling one category at a time; disabling a category deliberately leaves that portion of the stock client stale until a later normal refresh. `PUBLIC_PRESENTATION` remains the master switch for observer-visible look, buff, disease, and pet updates.
...
- The skill-union, borrowed-skill guard, SP synchronization, reusable inventory-presentation cache, cooldown-feedback coalescing, presentation-component, packet-batching, and surrounding Partner regression group passed 90 tests across 18 reports with 0 failures, 0 errors, and 0 skipped.
