...
- Switch presentation always sends the complete, optimized local and observer refresh; unsafe diagnostic switches that could deliberately leave the stock client stale were removed. The optional special effect is configurable, can be disabled, and is local-only by default. The redundant server replay of the trigger-skill effect is also disabled by default to reduce visual clutter.
...
- Double Partner invitation uses Agent E's conversation as the client-readiness boundary. The skill union and profile presentation inputs are prepared before the final confirmation dialog is sent; the session remains switch-locked until that dialog is acknowledged. Early Nimble Feet casts receive one themed loading message and later casts are silently handled. An optional bounded readiness delay can run after acknowledgement; the default is zero because stock v83 has no safe client-load acknowledgement and the visible NPC response is already a packet-order boundary. Completion clears the trigger cooldowns and announces that switching is ready. The flow never fakes a map transition.
...
`DOUBLE_PARTNER_READY_DELAY_MS` accepts 0-10000 ms. It is an optional minimum post-dialog settling window, not a client acknowledgement. `SWITCH_EFFECT_ID` accepts `-1` (disabled) or a byte-sized v83 effect ID. Useful candidates already exercised by the stock protocol are `7` (portal sound), `10` (recovery, default), `11` (buff), `17` (buff with SFX), and `8` (the legacy, much louder job-change effect). Avoid `9`, which is the quest-complete notification. `SWITCH_EFFECT_BROADCAST: false` prevents frequent switches from adding map-wide visual clutter. Nimble Feet remains the safest trigger because all four character families have a beginner variant and it uses the special-move packet path; the Recovery family (`1001`, `10001001`, `20001001`, `20011001`) is technically compatible but would replace normal Recovery use while a session is active.
