...
- Combat support special-move packet layout ownership moved to
  `server.agents.integration.cosmic.CosmicSupportSpecialMovePacketBuilder`.
...
- Combat skill label formatting now calls `AgentCombatDialogueReporter`
  ...
  fallback behavior.
- Support SPECIAL_MOVE packet layout tests now cover
  `CosmicSupportSpecialMovePacketBuilder` directly; the temporary
...
- Precise move-target navigation marker maintenance now lives in
  `server.agents.capabilities.navigation.AgentNavigationPreciseTargetService`;
  the precise-target flag rule after navigation target resolution is unchanged.
...
- Ranged retreat hold storage now lives in
  `server.agents.capabilities.combat.AgentRetreatHoldState`. `BotEntry`
...
- AoE reposition commitment storage now lives in
  `server.agents.capabilities.combat.AgentAoeRepositionState`.
...
- SPI/gateway extraction: Agent party joining now calls
  ...
  in `CosmicPartyGateway`.
- SPI/gateway extraction: same-party online-state publication now calls
  `AgentPartyGatewayRuntime`/`PartyGateway.publishAgentOnline` instead of
  constructing `PartyCharacter` and dispatching `PartyOperation.LOG_ONOFF`
...
- Death/recovery ownership: dead-state tick coordination moved from generic
  runtime to `capabilities.combat.AgentDeathTickCoordinator`, while respawn hook
...
- Command ownership: relog, logout, and away chat requests now coordinate through
  ...
  unchanged.
- Cosmic boundary ownership: support-skill packet opcode and byte layout now live
  in `integration.cosmic.CosmicSupportSpecialMovePacketBuilder`. The combat
  capability supplies skill intent through `CombatGateway`; generated bytes and
  normal packet-handler dispatch remain unchanged.
- Cosmic boundary ownership: buff consumable execution now calls
  `InventoryGateway.consumeUseItem`. `CosmicInventoryGateway` enters through
  `AgentUseItemBridge`, which rejects non-Agent characters before invoking the
  package-private `UseItemHandler` operation. Live slot, item ID, quantity,
  life-state, item choice, and one-item-per-tick behavior remain authoritative.
...
- Central scheduler foundation: `runtime.scheduler.AgentScheduler` is the
  ...
  failure metrics. `agents.scheduler.mode=legacy` remains the default; the old
  `agents.scheduler.central.enabled` compatibility property is still accepted,
