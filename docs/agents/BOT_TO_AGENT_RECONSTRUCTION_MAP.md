...
| `src/main/java/server/bots/BotCombatManager.java` | `server.agents.capabilities.combat` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; config and config command helpers, attack route, ammo counting, projectile hitbox geometry, fall-damage formula, skill classification, weapon policy, skill hitbox policy, hit-count policy, range/airborne-use policy, support safety policy, and skill-use affordability policy are Agent-owned |
