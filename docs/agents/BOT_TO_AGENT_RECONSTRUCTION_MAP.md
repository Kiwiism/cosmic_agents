...
| `src/main/java/server/bots/BotInventoryManager.java` | `server.agents.capabilities.inventory`, `looting`, `trade`, `server.agents.capabilities.dialogue.AgentItemQueryNormalizer` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; item query normalization, trade page/meso policy, sell-trash equipment protection policy, and ammo item-to-weapon classification are Agent-owned |
