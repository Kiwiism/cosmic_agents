...
| `src/main/java/server/bots/BotEntry.java` | `server.agents.runtime.AgentSession` and capability state objects | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; message queue state now lives in `AgentMessageQueueState`, queued message type is Agent-owned, and continuous ground-travel snapshots now use `AgentGroundTravelState` |
