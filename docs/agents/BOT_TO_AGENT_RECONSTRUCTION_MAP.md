...
| `src/main/java/server/bots/BotCombatManager.java` | `server.agents.capabilities.combat` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; config and config command helpers, attack route, ammo counting, projectile hitbox geometry, fall-damage formula, skill classification, weapon policy, skill hitbox policy, hit-count policy, range/airborne-use policy, support safety/party-filtering policy, skill-use affordability policy, mob-touch geometry, hitbox/monster intersection, scoring/target-score and AOE-cluster math, attack-plan tie-break ordering, and combat reply pools are Agent-owned |
