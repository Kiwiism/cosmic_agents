...
| `src/main/java/server/bots/BotCombatManager.java` | `server.agents.capabilities.combat`, `server.agents.capabilities.dialogue`, and `server.agents.integration.AgentBotCombatReportRuntime` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; config and config command helpers, attack route, attack-plan value model/scoring/selection, ammo counting, projectile hitbox geometry, fall-damage formula, skill classification/cache-signature/best-skill comparison/buff-blacklist, target eligibility/selection/opposite-facing pivoting/strike-point primary resolution, immediate projectile target policy, grind-target locality/scored-target ordering/grouping/occupancy-penalty/graph-path-cost policy, weapon policy, skill hitbox policy, hit-count policy, range/basic-reach/airborne-use policy, support safety/party-filtering/healer-ally/Dragon-Roar-threshold policy, support special-move packet layout, skill-use affordability policy, mob-touch/knockback/vector policy, hitbox/monster intersection, scoring/target-score/AOE-cluster default constants/AOE-vs-single-target/AOE-single-targeting/AOE-cluster-size math, attack-plan tie-break ordering, combat skill-label formatting, skill-buff debug report assembly, and combat reply pools are Agent-owned |
...
| `src/main/java/server/bots/BotInventoryManager.java` | `server.agents.capabilities.inventory`, `looting`, `trade`, `server.agents.capabilities.dialogue.AgentItemQueryNormalizer`, `server.agents.capabilities.dialogue.AgentDialogueCatalog`, `server.agents.capabilities.supplies.AgentPotionSharePolicy` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; item query normalization, trade page/meso policy, sell-trash equipment protection policy, ammo item-to-weapon classification, potion-share recovery scoring/slot eligibility, USE-item recovery/buff classification, and trade reply/result/error pools are Agent-owned |
...
| `src/main/java/server/bots/BotMovementManager.java` | `server.agents.capabilities.movement` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; cooldown/delay countdown math is Agent-owned |
...
| `src/main/java/server/bots/BotShopManager.java` | `server.agents.capabilities.shop`, `server.agents.capabilities.dialogue.AgentDialogueCatalog` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; shop resupply/shopping dialogue pools plus fixed sell-trash/shop visit/shortfall result messages are Agent-owned |
