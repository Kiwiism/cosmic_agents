...
| `src/main/java/server/bots/BotInventoryManager.java` | `server.agents.capabilities.inventory`, `looting`, `trade`, `server.agents.capabilities.dialogue.AgentItemQueryNormalizer` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; item query normalization and trade page/meso policy are Agent-owned |
