...
| `src/main/java/server/bots/BotChatManager.java` | `server.agents.capabilities.dialogue.AgentDialogueCatalog`, `server.agents.capabilities.dialogue.AgentChatCommandClassifier`, `server.agents.commands.AgentReplyQueue`, `server.agents.events` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; named random reply pools, reply queue, movement/follow/fidget classification, and supply-request classification are Agent-owned |
