...
| `src/main/java/server/bots/BotCombatManager.java` | `server.agents.capabilities.combat`, `server.agents.capabilities.dialogue`, and `server.agents.integration.AgentBotCombatReportRuntime` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; config and config command helpers plus removal of bot-side config pass-through wrappers, attack route, attack-plan value model/scoring/selection and plan orchestration runtime, basic attack target-selection/pivot policy and basic attack planning runtime, attack execution preflight policy and runtime orchestration, skill attack preflight/ammo/primary-target-resolution/packet-field policy and skill attack planning runtime, ammo counting/check-decision policy plus removal of bot-side ranged-ammo weapon and count-ammo wrappers, projectile hitbox geometry plus removal of bot-side passive projectile range/fallback/projectile wrappers, fall-damage formula plus removal of the bot-side formula wrapper, physical mob touch damage provider, mob/fall damage application runtime, and mob-touch polling runtime, skill classification/cache-signature/cache-bucket/heal-cache-stop/AoE-cache-score/support-buff-cache-eligibility/best-skill comparison/cached-skill-id ordering/buff-blacklist/skill-cache rebuild orchestration plus removal of bot-side skill-classification/cache wrappers, target eligibility/selection/opposite-facing pivoting/strike-point primary resolution/immediate projectile target/effective-primary/closest-alive/alive-in-range/heal-undead target policy and target-search runtime orchestration plus removal of bot-side target-eligibility, immediate-projectile, alive-in-range, heal-undead, opposite-facing, strike-point-primary, and target-selector pass-through wrappers, grind-target locality/scored-target ordering/grind-score dispatch/reachable-target selection/reachable-grind-target decision/no-entry overload removal/local-score-list construction/region-score construction/follow-local-score construction/grouping/occupancy-penalty/occupant-counting/graph-target helper removal/graph-path-cost policy/ground-foothold lookup runtime, weapon policy plus removal of bot-side Dragon Knight weapon-gate and damage-weapon wrappers, skill hitbox policy plus removal of bot-side strike-point anchor and calculate-hitbox wrappers, hit-count/Shadow-Partner multiplier policy plus removal of bot-side hit-count and multiplier wrappers, range/basic-reach/target-in-range/jumpable-target/airborne-use policy plus removal of bot-side basic-range/basic-reach, target-in-range, jumpable-target, can-use-plan, and unused airborne ranged-weapon wrappers, support safety/party-filtering/healer-ally/Dragon-Roar-threshold/support-buff candidate/readiness/failure-summary/outcome-summary/cooldown-summary/refresh-timing/cast-cooldown/skill-buff tick preflight/no-living-mobs preflight/support-buff tick orchestration/support-heal preflight/cast-trigger/self-threshold/missing-party-buff/heal-bounds policy/support-heal orchestration plus removal of bot-side support-heal threshold, Dragon Roar/healer-ally, and support-party wrappers, support special-move packet layout/dispatch executor plus removal of bot-side packet-builder and dispatch wrappers, attack-facing memory runtime plus removal of the bot-side stance-to-facing wrapper, action-lock cooldown runtime/action-state clearing/death-state entry plus removal of bot-side attack/move-window tick wrapper, alert stance/reset scheduling runtime plus removal of bot-side alert helper, mob-touch sweep/runtime plus removal of bot-side touch-bounds/check-memory wrappers, combat ammo shortage runtime plus removal of bot-side ammo-check side-effect wrapper, skill-use affordability policy plus removal of bot-side skill-use wrappers, mob-touch/knockback/vector policy plus removal of bot-side mob-touch inclusive-rectangle and knockback eligibility/direction/scaling wrappers, hitbox/monster intersection plus removal of bot-side target-hitbox wrappers, scoring/target-score/single-target score floor/AOE-cluster default constants/AOE-cluster-bias/AOE-vs-single-target/AOE-single-targeting/AOE-cluster-size/AOE-reposition preflight/AOE-reposition math/travel-cost/nearest-cluster selection/runtime orchestration plus removal of bot-side scoring and AOE-cluster helper, PlanScore, AoE scoring, and local selector/planner pass-through wrappers, attack-plan tie-break ordering, combat skill-label formatting plus removal of bot-side label wrapper, combat debug-stat and skill-buff debug report assembly plus removal of bot-side debug-decision recording and debug-lines wrappers, and combat reply pools are Agent-owned |
