...
| `src/main/java/server/bots/BotCombatManager.java` | `server.agents.capabilities.combat` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; config and config command helpers, attack route, ammo counting, projectile hitbox geometry, fall-damage formula, skill classification/cache-signature/best-skill comparison, target eligibility/selection, immediate projectile target policy, grind-target locality policy, weapon policy, skill hitbox policy, hit-count policy, range/basic-reach/airborne-use policy, support safety/party-filtering/healer-ally policy, skill-use affordability policy, mob-touch/knockback policy, hitbox/monster intersection, scoring/target-score/AOE-cluster/AOE-vs-single-target math, attack-plan tie-break ordering, and combat reply pools are Agent-owned |
