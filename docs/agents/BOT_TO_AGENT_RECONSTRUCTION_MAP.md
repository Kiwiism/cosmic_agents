...
| `src/main/java/server/bots/BotNavigationManager.java` | `server.agents.capabilities.navigation` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; navigation-facing physics helper calls for first-climbable rope Y and walkable endpoint-step policy route through `AgentNavigationPhysicsService` |
