...
| `src/main/java/server/bots/BotManager.java` | `server.agents.runtime`, `commands`, `events`, capability orchestrators | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; ownerless/offline-leader tick branching now delegates to `AgentOwnerlessTickService`, preserving following clear, map-change grounding, explicit move-target pass-through, and idle fallback ordering. Tick-time leader-motion observation and farm-anchor map-change cleanup now delegate to `AgentTickStateMaintenanceService`. Tracked map-change tick handling now delegates to `AgentMapTransitionService` |
