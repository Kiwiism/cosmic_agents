...
| `src/main/java/server/bots/BotNavigationManager.java` | `server.agents.capabilities.navigation` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; navigation-facing physics helper calls for first-climbable rope Y, walkable endpoint-step policy, and graph build walk-region lookup lifecycle route through `AgentNavigationPhysicsService`; movement capability ground-target, mob-avoidance, follow-target, path logger, script move-target, and navigation debug overlay region classification route through `AgentNavigationRegionService`; path logger, script move-target, and navigation debug overlay path searches route through `AgentNavigationPathService` |
