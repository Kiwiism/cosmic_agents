...
| `src/main/java/server/bots/BotEquipManager.java` | `server.agents.capabilities.equipment` | `SPLIT_TO_MULTIPLE_AGENT_MODULES`; map-damage benchmark snapshot/selection now lives in `AgentMapDamageProfile`, while optimizer and equip execution remain temporary bot seams |
