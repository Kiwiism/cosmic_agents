...
function inventorySearchSubtype(item:any,storageMode:boolean){
  if(storageMode)return "";
  const itemId=Number(item?.itemid||0);
  const itemCategory=itemId>0?Math.floor(itemId/1_000_000):Number(item?.inventorytype||0);
  return inventoryItemTypes[itemCategory-1]||"";
}
...
function CharacterStats({notify,focusCharacter,onCharacterSelect}:{notify:Notify;focusCharacter?:number;onCharacterSelect?:(id:number)=>void}){
  ...
  const [skills,setSkills]=useState<any[]>([]);const [equippedItems,setEquippedItems]=useState<any[]>([]);const [jobCatalog,setJobCatalog]=useState<any[]>([]);
  useEffect(()=>{api<Page<any>>("/api/accounts?sort=name&direction=asc&page=0&size=200").then(r=>setAccounts(r.items))},[]);
  useEffect(()=>{api<any[]>("/api/catalog/jobs").then(setJobCatalog)},[]);
  ...
  async function deleteSkill(skill:any,reason:string){if(!character)return;await api(`/api/characters/${character.id}/skills/${skill.skillid}`,{method:"PATCH",body:JSON.stringify({skillLevel:0,masterLevel:0,expiration:Number(skill.expiration??-1),reason})})}
  ...
  async function maxJobSkills(jobId:number){if(!character)return;const jobSkills=skills.filter(skill=>Number(skill.job_id||0)===jobId);const book=skillBook(jobId);const pools=spArray(form.sp);pools[book]=0;setSkills(rows=>rows.map(row=>Number(row.job_id||0)===jobId?{...row,skilllevel:Number(row.max_level||0),savedSkilllevel:Number(row.max_level||0),masterlevel:Math.max(Number(row.masterlevel||0),Number(row.max_level||0))}:row));setForm((previous:any)=>({...previous,sp:pools.join(",")}));await Promise.all(jobSkills.map(skill=>saveSkill(skill,Number(skill.max_level||0),`Maxed job ${jobId} skills from AP/SP page`)));await saveSp(pools);notify(`Maxed job ${jobId} skills`)}
  async function deleteJobSkills(jobId:number){if(!character)return;const jobSkills=skills.filter(skill=>Number(skill.job_id||0)===jobId);const book=skillBook(jobId);const pools=spArray(form.sp);const refunded=jobSkills.reduce((sum,skill)=>sum+Number(skill.savedSkilllevel??skill.skilllevel??0),0);pools[book]+=refunded;setSkills(rows=>rows.filter(row=>Number(row.job_id||0)!==jobId));setForm((previous:any)=>({...previous,sp:pools.join(",")}));await Promise.all(jobSkills.map(skill=>deleteSkill(skill,`Deleted job ${jobId} skill row from AP/SP page`)));await saveSp(pools);notify(`Deleted job ${jobId}`)}
  ...
  async function addJobSkills(jobId:number){if(!jobId||skills.some(skill=>Number(skill.job_id||0)===jobId))return;const jobInfo=jobCatalog.find(row=>Number(row.job_id)===jobId);const page=await api<Page<Entity>>(`/api/catalog/search?type=SKILL&jobId=${jobId}&page=0&size=500&sort=id`);const nextSkills=page.items.map(row=>({skillid:row.entity_id,skill_name:row.name,job_id:jobId,job_name:jobInfo?.job_name||row.job_name||`Job ${jobId}`,max_level:row.level_value||30,properties_json:row.properties_json,skilllevel:0,savedSkilllevel:0,masterlevel:0,expiration:-1}));setSkills(rows=>[...rows,...nextSkills.filter(skill=>!rows.some(row=>Number(row.skillid)===Number(skill.skillid)))].sort((a,b)=>Number(a.job_id||0)-Number(b.job_id||0)||Number(a.skillid||0)-Number(b.skillid||0)));notify(`Added ${jobInfo?.job_name||`Job ${jobId}`} skill card`)}
  const columns=[0,1,2,3,4].map(tier=>{const jobs=Array.from(new Set(skills.filter(s=>skillBook(s.job_id)===tier).map(s=>Number(s.job_id||0)))).sort((a,b)=>a-b);const addable=jobCatalog.filter(job=>skillBook(job.job_id)===tier&&!jobs.includes(Number(job.job_id))).sort((a,b)=>Number(a.job_id)-Number(b.job_id));return {tier,label:["Beginner","1st Job","2nd Job","3rd Job","4th Job"][tier],jobs,addable}});
    ...
    {!character?<Empty text="Choose an account and character to edit AP, stats, location, and skills"/>:<div className="panel ap-sp-panel"><PanelTitle title={`${character.account_name} → ${character.name}`} subtitle="Account must be offline before character stat or skill edits are saved"/>
      ...
      <section className="skill-board"><div className="skill-board-head"><div><h3>SP allocation and skills</h3><p>Use + or - to spend from or return to the matching SP pool. Typing a skill level is an admin override.</p></div></div>
        {skills.length===0?<div className="skill-empty-state"><strong>No skill rows found for this character yet.</strong><span>Import or refresh the WZ catalog from the dashboard. If this character already has saved skills, restart the API so the fallback saved-skill lookup is active.</span></div>
          :<div className="skill-columns">{columns.map(column=><article className="skill-column" key={column.tier}>
            <div className="skill-column-head">
              <div className="skill-column-top"><strong>{column.label}</strong><label className="unused-sp-editor"><input type="number" min="0" value={spPools[column.tier]||0} onChange={e=>{const pools=spArray(form.sp);pools[column.tier]=Math.max(0,Number(e.target.value));setForm((previous:any)=>({...previous,sp:pools.join(",")}))}} onBlur={e=>setSpPool(column.tier,Number(e.target.value))} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();e.currentTarget.blur()}}}/><span>unused SP</span></label></div>
              <div className="skill-tier-actions"><button type="button" className="text-button compact" onClick={()=>maxTierSkills(column.tier)}>Max all</button><button type="button" className="text-button compact" onClick={()=>resetTierSkills(column.tier)}>Reset all</button></div>
              {column.tier>0&&<select className="skill-add-job-select" value="" onChange={event=>{const jobId=Number(event.target.value);event.currentTarget.value="";addJobSkills(jobId)}}>
                <option value="">Add {column.label} job...</option>
                {column.addable.map(job=><option key={job.job_id} value={job.job_id}>{job.job_name||`Job ${job.job_id}`} ({job.job_id})</option>)}
              </select>}
            </div>
            {column.jobs.map(jobId=>{const jobSkills=skills.filter(s=>Number(s.job_id||0)===jobId);const firstSkill=jobSkills[0];return <div className="skill-job-card" key={jobId}>
              <h4><span><JobBadge jobId={jobId} skillId={Number(firstSkill?.skillid||0)}/>{firstSkill?.job_name||`Job ${jobId}`} <code>{jobId}</code></span><span className="skill-job-actions"><button type="button" className="text-button compact" onClick={()=>maxJobSkills(jobId)}>Max all</button><button type="button" className="text-button compact" onClick={()=>resetJobSkills(jobId)}>Reset</button><button type="button" className="text-button compact danger-text" onClick={()=>deleteJobSkills(jobId)}>Delete</button></span></h4>
              {jobSkills.map(skill=><div className="skill-edit-row" key={skill.skillid}><span><strong>{skill.skill_name||`Skill ${skill.skillid}`}</strong><code>{skill.skillid} | max {skill.max_level||0}</code></span><input type="number" min="0" max={skill.max_level||0} value={skill.skilllevel||0} onChange={e=>setSkills(rows=>rows.map(row=>row.skillid===skill.skillid?{...row,skilllevel:Number(e.target.value)}:row))} onBlur={e=>setSkillLevel(skill,Number(e.target.value),false)}/><div className="skill-stepper"><button type="button" className="icon-link" onClick={()=>setSkillLevel(skill,Number(skill.skilllevel||0)-1,true)}>-</button><button type="button" className="icon-link" onClick={()=>setSkillLevel(skill,Number(skill.skilllevel||0)+1,true)}>+</button></div></div>)}
            </div>})}
          </article>)}</div>}
      </section>
    </div>}</>
...
function Inventory({notify,focusCharacter,onCharacterSelect,onOpen,jump,onInspectorChange}:{notify:Notify;focusCharacter?:number;onCharacterSelect?:(id:number)=>void;onOpen:(type:string,id:number)=>void;jump:(x:JumpTarget)=>void;onInspectorChange:(size:"none"|"standard"|"wide")=>void}){
  ...
  useEffect(()=>{if(!character?.accountid||!accounts.length)return;const account=accounts.find(row=>Number(row.id)===Number(character.accountid));if(account&&Number(browseAccount?.id)!==Number(account.id)){setBrowseAccount(account);api<any[]>(`/api/accounts/${account.id}/characters`).then(setAccountCharacters)}},[character?.accountid,accounts.length]);
  async function load(){if(!character)return{inventoryRows:[],storageRows:[]};const [inventoryRows,storageRows]=await Promise.all([api<any[]>(`/api/characters/${character.id}/inventory`),api<any[]>(`/api/accounts/${character.accountid}/storage?world=${character.world}`)]);setItems(inventoryRows);setStorage(storageRows);return{inventoryRows,storageRows}}
  ...
  async function saveInventoryMeso(value:number){const updated=await api<any>(`/api/characters/${character.id}/meso`,{method:"PATCH",body:JSON.stringify({meso:value,reason:"Updated inventory mesos through Console"})});setCharacter(updated);setAccountCharacters(rows=>rows.map(row=>Number(row.id)===Number(updated.id)?{...row,meso:updated.meso}:row));notify("Inventory mesos updated")}
  ...
  async function transferInventoryToStorage(item:any){const position=firstStorageSlot();if(position==null){notify("Storage is full");return}
    ...
    const refreshed=await load();const source=refreshed.inventoryRows.find(row=>Number(row.inventorytype)===Number(item.inventorytype)&&Number(row.position)===Number(item.position)&&Number(row.itemid)===Number(item.itemid));
    if(!source)throw new Error("Inventory item changed before the storage transfer completed");
    await api(`/api/characters/${character.id}/inventory/${source.inventoryitemid}?reason=Moved%20to%20storage`,{method:"DELETE"});notify("Item moved to storage");setEditing(null);const finalRows=await load();setEditingStorage(finalRows.storageRows.find(row=>Number(row.position)===Number(stored.position)&&Number(row.itemid)===Number(stored.itemid))||null)}
  async function transferStorageToInventory(item:any){const type=item.inventorytype||Math.floor(item.itemid/1_000_000);const position=firstInventorySlot(type);if(position==null){notify("Inventory category is full");return}
    ...
    const refreshed=await load();const source=refreshed.storageRows.find(row=>Number(row.position)===Number(item.position)&&Number(row.itemid)===Number(item.itemid));
    if(!source)throw new Error("Storage item changed before the inventory transfer completed");
    await api(`/api/accounts/${character.accountid}/storage/${source.inventoryitemid}?world=${character.world}&reason=Moved%20to%20inventory`,{method:"DELETE"});notify("Item moved to inventory");setEditingStorage(null);const finalRows=await load();setEditing(finalRows.inventoryRows.find(row=>Number(row.inventorytype)===Number(type)&&Number(row.position)===Number(added.position)&&Number(row.itemid)===Number(added.itemid))||null)}
  ...
  return <><div className="inventory-account-toolbar"><SearchInput value={accountQuery} setValue={setAccountQuery} placeholder="Search account, character or ID"/>
    ...
    {!character?<Empty text="Choose an account and character to edit inventory and storage"/>:<>{groups.map(group=><article className="panel inventory-panel" key={group.type}><div className="inventory-panel-head"><PanelTitle title={["","Equip","Use","Setup","Etc","Cash"][group.type]} subtitle="Click an item to edit. Drag onto an empty slot to move."/>
      ...
      <article className="panel inventory-panel"><div className="storage-heading"><PanelTitle title="Account storage" subtitle={`World ${character.world}`}/>
        <div className="storage-tools">{Number(storage[0]?.slots||0)<48&&<button className="secondary" onClick={expandStorage}>Expand storage to 48 slots</button>}
          <StorageMesoEditor label="Inventory mesos" value={Number(character.meso||0)} save={saveInventoryMeso}/><StorageMesoEditor label="Storage mesos" value={Number(storage[0]?.meso||0)} save={saveStorageMeso}/><div className="inventory-transfer-zone" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();const raw=e.dataTransfer.getData("storageItem");if(raw)transferStorageToInventory(JSON.parse(raw))}}>Drag here to put in inventory</div></div></div>
    ...
    {editing&&character&&<ItemEditor key={`${editing.newItem?"new":editing.inventoryitemid}-${editing.position}`} item={editing} characterId={character.id} close={()=>setEditing(null)} saved={async savedItem=>{const {inventoryRows}=await load();if(!savedItem){setEditing(nearestInventoryItem(inventoryRows,editing));return}setEditing(inventoryRows.find(row=>Number(row.inventorytype)===Number(savedItem.inventorytype)&&Number(row.position)===Number(savedItem.position)&&Number(row.itemid)===Number(savedItem.itemid))||null)}} notify={notify} duplicate={duplicate} onOpen={onOpen} jump={jump}/>}
    {editingStorage&&character&&<ItemEditor key={`storage-${editingStorage.newItem?"new":editingStorage.inventoryitemid}-${editingStorage.position}`} item={editingStorage} characterId={character.id} accountId={character.accountid} world={character.world} close={()=>setEditingStorage(null)} saved={async savedItem=>{const {storageRows}=await load();if(!savedItem){setEditingStorage(nearestStorageItem(storageRows,editingStorage));return}setEditingStorage(storageRows.find(row=>Number(row.position)===Number(savedItem.position)&&Number(row.itemid)===Number(savedItem.itemid))||null)}} notify={notify} duplicate={async()=>{}} allowDuplicate={false} onOpen={onOpen} jump={jump} storageMode/>}</>
}
...
function EquipmentAppearance({notify,focusCharacter,onCharacterSelect,onOpen,jump,onInspectorChange}:{notify:Notify;focusCharacter?:number;onCharacterSelect?:(id:number)=>void;onOpen:(type:string,id:number)=>void;jump:(x:JumpTarget)=>void;onInspectorChange:(size:"none"|"standard"|"wide")=>void}){
  ...
  return <><div className="inventory-account-toolbar"><SearchInput value={accountQuery} setValue={setAccountQuery} placeholder="Search account, character or ID"/><select value={worldFilter} onChange={e=>setWorldFilter(e.target.value)}><option value="all">All worlds</option>{accountWorlds.map(world=><option value={world} key={world}>World {world}</option>)}</select></div>
    ...
    {editing&&character&&<ItemEditor key={`${editing.newItem?"new":editing.inventoryitemid}-${editing.position}`} item={editing} characterId={character.id} close={()=>setEditing(null)} saved={async savedItem=>{const rows=await load();if(!savedItem){setEditing(null);return}setEditing(rows.find(row=>Number(row.position)===Number(savedItem.position)&&Number(row.itemid)===Number(savedItem.itemid))||null)}} notify={notify} duplicate={async()=>{}} allowDuplicate={false} onOpen={onOpen} jump={jump}/>}</>
}
...
function ItemEditor({item,characterId,accountId,world,close,saved,notify,duplicate,onOpen,jump,allowDuplicate=true,storageMode=false}:{item:any;characterId:number;accountId?:number;world?:number;close:()=>void;saved:(item?:any)=>void|Promise<void>;notify:Notify;duplicate:(x:any)=>void;onOpen:(t:string,id:number)=>void;jump:(x:JumpTarget)=>void;allowDuplicate?:boolean;storageMode?:boolean}){
  ...
  const editSection=<section className="inventory-edit-section"><h3>{item.newItem?"New item values":"Current saved item values"}</h3>
    <Autocomplete type="ITEM" subtype={inventorySearchSubtype(item,storageMode)} value={selected} onSelect={setSelected} placeholder={item.newItem?storageMode?"Search any item":"Search compatible item":"Search to replace with another compatible item"}/>
...
function StorageMesoEditor({label,value,save}:{label:string;value:number;save:(v:number)=>void}){
  const [editing,setEditing]=useState(false);const [draft,setDraft]=useState(String(value));useEffect(()=>setDraft(String(value)),[value]);
  function commit(){const next=Math.max(0,parseNumberInput(draft));setEditing(false);if(next!==value)save(next)}
  return <div className="storage-meso"><Coins/><span><small>{label}</small>{editing
