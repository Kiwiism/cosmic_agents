...
import server.agents.commands.townlife.AgentTownLifeAmbientCommandService;
...
public final class TownLifeCommand extends Command {
    {
        setDescription("Show TownLife diagnostics or control bounded observation harnesses.");
    }
    ...
    public void execute(Client client, String[] params) {
        try {
            ...
            if (params != null && params.length > 0 && "test".equalsIgnoreCase(params[0])) {
                lines = AgentTownLifeTestService.execute(
                        client.getPlayer(), params, System.currentTimeMillis());
            } else if (params != null && params.length > 0
                    && "ambient".equalsIgnoreCase(params[0])) {
                lines = AgentTownLifeAmbientCommandService.execute(
                        client.getPlayer(), params, System.currentTimeMillis());
            } else {
                lines = AgentTownLifeDiagnostics.lines(params);
            }
