...
public record AgentVictoriaLevel15PlanCard(
        int schemaVersion,
        String planId,
        String title,
        String status,
    ...
    public AgentVictoriaLevel15PlanCard {
        if (schemaVersion <= 0 || blank(planId) || blank(title) || blank(status) || blank(catalogId)
                || entryCriteria == null || requiredCapabilityIds == null || requiredCapabilityIds.isEmpty()
                || requiredCapabilityIds.stream().anyMatch(AgentVictoriaLevel15PlanCard::blank)
    ...
    public record EntryCriteria(
        ...
        public EntryCriteria {
            if (mapId <= 0 || jobId < 0 || minimumLevel <= 0 || maximumLevel < minimumLevel
                    ...
                    || startVariantIds.stream().anyMatch(AgentVictoriaLevel15PlanCard::blank)) {
    ...
    public record Stage(String stageId, List<String> capabilityIds, String postcondition) {
        public Stage {
            if (blank(stageId) || capabilityIds == null || capabilityIds.isEmpty()
                    || capabilityIds.stream().anyMatch(AgentVictoriaLevel15PlanCard::blank)
                    || blank(postcondition)) {
