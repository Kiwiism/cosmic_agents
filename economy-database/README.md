...
PostgreSQL executes `migrations/V001` through `V016` in lexical order only while initializing an
...
V016 schema contract before creating a run. The PostgreSQL JDBC pool is independent of Cosmic's
MySQL pool.

The migrations are verified against a clean PostgreSQL data directory during implementation;
all V001-V016 scripts and the application `EconomyDatabaseVerifier` must accept the result.
