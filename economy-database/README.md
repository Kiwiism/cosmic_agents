...
PostgreSQL executes `migrations/V001` through `V011` in lexical order only while initializing an
...
V011 schema contract before creating a run. The PostgreSQL JDBC pool is independent of Cosmic's
MySQL pool.

The migrations were executed from a clean PostgreSQL 16.4 data directory during implementation;
all V001-V011 scripts completed and the application `EconomyDatabaseVerifier` accepted the result.
