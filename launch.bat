...
java %JAVA_OPTS% -Xmx2048m -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=logs\heapdumps -XX:+ExitOnOutOfMemoryError -XX:ErrorFile=logs\crash\hs_err_pid%%p.log -Xlog:gc*,safepoint:file=logs/gc/gc.log:time,uptime,level,tags:filecount=10,filesize=20M -Dagents.scheduler.mode=central-sequential -Dwz-path=wz -jar target\Cosmic.jar
pause
